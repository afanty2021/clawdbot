...
- 插件: 安装在插件目录运行 `npm install --omit=dev`；运行时依赖必须放在 `dependencies` 中。避免在 `dependencies` 中使用 `workspace:*`（npm 安装会失败）；将 `moltbot` 放在 `devDependencies` 或 `peerDependencies` 中（运行时通过 jiti 别名解析 `clawdbot/plugin-sdk`）。
- 从 `https://molt.bot/*` 提供的安装程序: 位于同级仓库 `../molt.bot` (`public/install.sh`, `public/install-cli.sh`, `public/install.ps1`)。
...
- 当 Peter 要求链接时，回复完整的 `https://docs.molt.bot/...` URL（不是根相对路径）。
- 当你接触文档时，回复末尾附上你引用的 `https://docs.molt.bot/...` URL。
- README (GitHub): 保持绝对的文档 URL (`https://docs.molt.bot/...`)，以便链接在 GitHub 上工作。
...
- 实时测试（真实密钥）: `CLAWDBOT_LIVE_TEST=1 pnpm test:live`（仅 Moltbot）或 `LIVE=1 pnpm test:live`（包括 provider 实时测试）。Docker: `pnpm test:docker:live-models`, `pnpm test:docker:live-gateway`。入门 Docker E2E: `pnpm test:docker:onboard`。
...
## 安全和配置提示
- Web provider 在 `~/.clawdbot/credentials/` 存储凭据；如果注销则重新运行 `moltbot login`。
...
## 故障排除
- Rebrand/迁移问题或遗留配置/服务警告: 运行 `moltbot doctor`（参见 `docs/gateway/doctor.md`）。
...
- Gateway 目前仅作为 menubar app 运行；没有安装单独的 LaunchAgent/helper 标签。通过 Moltbot Mac app 或 `scripts/restart-mac.sh` 重启；要验证/kill 使用 `launchctl print gui/$UID | grep moltbot` 而不是假设固定标签。**在 macOS 上调试时，通过 app 启动/停止 gateway，而不是临时 tmux 会话；在交接前终止任何临时隧道。**
...
- 当被要求打开"会话"文件时，打开 `~/.clawdbot/agents/<agentId>/sessions/*.jsonl` 下的 Pi 会话日志（使用系统提示 Runtime 行中的 `agent=<id>` 值；最新的，除非给出了特定 ID），而不是默认的 `sessions.json`。如果需要来自另一台机器的日志，通过 Tailscale SSH 并在那里读取相同的路径。
- 不要通过 SSH 重建 macOS app；重建必须直接在 Mac 上运行。
- 永远不要向外部消息传递表面（WhatsApp、Telegram）发送流式/部分回复；只有最终回复应该在那里传递。流式/工具事件可能仍然会去内部 UIs/控制通道。
- Voice wake 转发提示:
  - 命令模板应该保持 `moltbot-mac agent --message "${text}" --thinking low`；`VoiceWakeForwarder` 已经 shell 转义 `${text}`。不要添加额外的引号。
  - launchd PATH 是最小的；确保 app 的 launch agent PATH 包括标准系统路径加上你的 pnpm bin（通常是 `$HOME/Library/pnpm`），以便通过 `moltbot-mac` 调用时 `pnpm`/`moltbot` 二进制文件解析。
